#\!/bin/bash
# Прямое исправление категорий на сервере

echo "🔧 Прямое исправление логики категорий"
echo "======================================"

# Создаем исправленный блок ABC локально и копируем на сервер
cat > fixed_abc_block.py << 'PYTHON_EOF'
#\!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
scp fixed_abc_block.py root@217.114.1.117:/opt/inventory_system/

echo "🔧 Запускаем исправление на сервере..."
ssh root@217.114.1.117 "cd /opt/inventory_system && python3 fixed_abc_block.py"
...
echo "   - Петли и направляющие"
echo "   - и другие осмысленные категории"
echo ""
echo "📋 Проверить результат: ssh root@217.114.1.117 'tail -f /opt/inventory_system/webhook_8502.log'"

# Очистка
rm -f fixed_abc_block.py
