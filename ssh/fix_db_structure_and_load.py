...
# -*- coding: utf-8 -*-
"""
Исправление структуры БД и полная загрузка данных
"""
...
def fix_database_structure():
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        # Проверяем текущую структуру таблицы sales
        cursor.execute("PRAGMA table_info(sales)")
        current_columns = [col[1] for col in cursor.fetchall()]
        print(f"📋 Текущие колонки sales: {current_columns}")
        ...
        if 'category_path' not in current_columns:
            missing_columns.append('category_path')
            cursor.execute("ALTER TABLE sales ADD COLUMN category_path TEXT")
            print("✅ Добавлена колонка 'category_path'")
        
        if not missing_columns:
            print("✅ Структура таблицы sales уже корректна")
...
def load_full_data():
    """Загружает полные данные из файла остатков"""
    ...
    # Загружаем продажи в БД с новой структурой
    with sqlite3.connect(accumulator.db_path) as conn:
...
def verify_data():
    ...
    with sqlite3.connect(accumulator.db_path) as conn:
        ...
        # Проверяем склады
        cursor.execute("SELECT DISTINCT warehouse FROM stock LIMIT 10")
        warehouses = cursor.fetchall()
        print("🏪 Склады:")
        for wh, in warehouses:
            print(f"   - {wh}")
    
    # Тестируем через accumulator
    print("\n🧪 ТЕСТИРОВАНИЕ ЧЕРЕЗ ACCUMULATOR:")
    print("-" * 30)
    ...
    try:
        ...
        print(f"📊 get_sales_data(): {len(sales_data)} записей")
        print(f"📦 get_latest_stock(): {len(stock_data)} записей")
        
        if not sales_data.empty:
            print(f"📂 Есть category_path: {'category_path' in sales_data.columns and sales_data['category_path'].notna().any()}")
            
        if not sales_data.empty and not stock_data.empty:
            print("✅ ВСЕ АНАЛИТИЧЕСКИЕ ФУНКЦИИ ДОЛЖНЫ РАБОТАТЬ!")
        else:
            print("❌ Данные не загружены через accumulator")
            
    except Exception as e:
        print(f"❌ Ошибка тестирования accumulator: {e}")
    
    return True

def main():
    """Основная функция"""
    print("🚀 ПОЛНОЕ ИСПРАВЛЕНИЕ СИСТЕМЫ АНАЛИТИКИ")
    print("=" * 60)
    
    try:
        # 1. Исправляем структуру БД
        if not fix_database_structure():
            print("❌ Ошибка исправления структуры БД")
            return False
