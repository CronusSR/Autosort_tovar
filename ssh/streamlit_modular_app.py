...
try:
    from branch_analytics import integrate_branch_analytics
    BRANCH_ANALYTICS_AVAILABLE = True
except ImportError:
...
def init_system():
    ...
    load_ads_from_json_to_system(system)
        
    return system

def load_ads_from_json_to_system(system):
    """Загружает данные ADS из JSON файлов в основную систему"""
    import os
    import json
    import pandas as pd
    ...
    try:
        ...
        if os.path.exists(ads_path):
            ...
            if 'branches' in combined_data and len(combined_data['branches']) > 0:
                # Создаем объединенный DataFrame для совместимости
                all_ads_data = []
                
                for branch_name, branch_info in combined_data['branches'].items():
                ...
                if all_ads_data:
                    # Создаем DataFrame и сохраняем в систему
                    ads_df = pd.DataFrame(all_ads_data)
                    
                    # Группируем по номенклатуре (берем максимальный ADS)
                    ...
                    system.calculated_ads = grouped_ads
                    # Также сохраняем оригинальные данные
                    system.ads_by_branches = ads_df
                    
    except Exception as e:
        pass  # Если не удалось загрузить, продолжаем без ADS
...
def ads_calculation_page_updated(system):
    ...
    # Добавляем поддержку единого файла продаж
    try:
        from single_file_ads_processor import SingleFileADSProcessor
        
        st.subheader("📂 Способы загрузки данных")
        
        loading_method = st.radio(
            "Выберите способ загрузки:",
            ["📄 Единый файл со всеми филиалами", "📁 Множественные файлы (старый способ)"],
            key="loading_method"
        )
        
        if loading_method == "📄 Единый файл со всеми филиалами":
            st.info("💡 Новый рекомендуемый способ: один файл содержит данные всех филиалов")
            processor = SingleFileADSProcessor()
            processor.create_streamlit_interface()
            return  # ОСТАНАВЛИВАЕМСЯ ЗДЕСЬ - больше не показываем старый интерфейс
            
        else:
            st.info("📁 Старый способ: загрузка отдельных файлов для каждого филиала")
            try:
                from integration_patch import add_multiple_files_interface_to_existing
        ...
        st.warning("⚠️ Модуль обработки единого файла не найден. Используется только множественная загрузка.")
        try:
            from integration_patch import add_multiple_files_interface_to_existing
    ...
    **🔢 ФОРМУЛА ADS:**
    ...
    - **Формула:** ADS = (среднее значение от M4:AB4) / 30.5
...
def add_json_export_section(system):
    ...
    # Добавляем аналитику филиалов в конце страницы ADS - ВСЕГДА показываем
    st.markdown("---")
    st.subheader("📈 Детальная аналитика по филиалам")
    
    # Проверяем наличие данных филиалов
    import os
    
    # Отладочная информация
    st.write(f"🔍 **Диагностика:**")
    st.write(f"- Файл данных существует: {'✅' if os.path.exists('ads/combined_ads_data.json') else '❌'}")
    st.write(f"- Модуль аналитики доступен: {'✅' if BRANCH_ANALYTICS_AVAILABLE else '❌'}")
    
    if os.path.exists('ads/combined_ads_data.json'):
        # Читаем и показываем информацию о данных
        try:
            import json
            with open('ads/combined_ads_data.json', 'r', encoding='utf-8') as f:
                data = json.load(f)
            
            st.success(f"✅ **Найдены данные для {data['branches_count']} филиалов**")
            st.write(f"Последнее обновление: {data['updated']}")
            
            # Показываем список филиалов
            st.write("**Доступные филиалы:**")
            for branch_key, branch_info in data['branches'].items():
                st.write(f"• {branch_key}: {branch_info['items_count']} товаров")
            
        except Exception as e:
            st.error(f"❌ Ошибка чтения данных: {e}")
        
        st.info("📊 Детальная аналитика включает ABC анализ, рейтинги товаров и анализ оборачиваемости")
        
        if BRANCH_ANALYTICS_AVAILABLE:
            with st.expander("🔍 Открыть аналитику филиалов", expanded=False):
                try:
            ...
            st.write("Проверьте наличие файла branch_analytics.py")
    else:
        st.warning("⚠️ Данные филиалов не найдены")
        st.write("Сначала загрузите единый файл продаж выше ⬆️")
...
def main():
    ...
    with st.sidebar:
        ...
        # Аналитика филиалов теперь интегрирована в ADS расчет
