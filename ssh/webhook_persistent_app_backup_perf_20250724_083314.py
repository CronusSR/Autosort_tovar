...
with tab6:
    ...
    if not sales_data.empty:
        ...
        # Подготовка данных для тепловой карты с поддержкой любого количества месяцев
        sales_data_copy = sales_data.copy()
        sales_data_copy['date_parsed'] = pd.to_datetime(sales_data_copy['date'])
        sales_data_copy['year_month'] = sales_data_copy['date_parsed'].dt.to_period('M').astype(str)
        sales_data_copy['day'] = sales_data_copy['date_parsed'].dt.day
        
        # Группируем по году-месяцу и дню
        heatmap_data = sales_data_copy.groupby(['year_month', 'day'])['amount'].sum().reset_index()
        
        if not heatmap_data.empty:
            # Создаем pivot таблицу
            heatmap_pivot = heatmap_data.pivot(index='day', columns='year_month', values='amount')
            
            # Заполняем пустые значения нулями
            heatmap_pivot = heatmap_pivot.fillna(0)
            
            # Сортируем колонки по хронологии
            sorted_columns = sorted(heatmap_pivot.columns)
            heatmap_pivot = heatmap_pivot[sorted_columns]
            
            # Создаем тепловую карту
            fig_heatmap = px.imshow(
                heatmap_pivot,
                labels=dict(x="Период (год-месяц)", y="День месяца", color="Выручка (₸)"),
                title=f"Тепловая карта продаж ({len(sorted_columns)} месяцев)",
            ...
            # Поворачиваем подписи на оси X если месяцев много
            if len(sorted_columns) > 12:
                fig_heatmap.update_layout(
            ...
            st.info(f"📊 Период: {sorted_columns[0]} - {sorted_columns[-1]} ({len(sorted_columns)} месяцев)")
        else:
            st.warning("⚠️ Недостаточно данных для построения тепловой карты")
